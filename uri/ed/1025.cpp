...
int main(){
    ...
    while(cin>>n>>q and n+q){
        ...
        sort(marm.begin(), marm.begin()+n);
        FOR(q){
            cin>>v;
            if(*(lb = lower_bound(marm.begin(), marm.begin()+n, v)) == v){
                cout<<v<<" found at "<<lb-marm.begin()+1<<endl;
            }
            else cout<<v<<" not found"<<endl;
        }
    }
    return 0;
}
